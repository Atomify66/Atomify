...
app.get("/auth/google/callback",
  ...
  (req, res) => {
    // Successful authentication — save userId explicitly before redirect
...
app.get("/user", (req, res) => {
  ...
  // Temporary diagnostic: log every /user hit with referrer + cookie presence,
  // so we can see when admin.html (or anything else) is asking without a session.
  const hasCookie = !!(req.headers.cookie && req.headers.cookie.includes('connect.sid'));
  console.log(`[/user] referer=${req.headers.referer || '-'} cookie=${hasCookie} sid=${req.sessionID} session.userId=${req.session.userId || '-'} req.user=${req.user ? req.user.id : '-'}`);
  if (!req.session.userId && !req.user) {
    return res.status(401).json({ error: "Not authenticated" });
  }
...
app.use('/app', express.static(path.join(__dirname, "app"), {
  setHeaders: (res, path) => {
    ...
    // SW must never be cached — browsers need to re-fetch to detect updates
    if (path.includes('sw.js')) {
      res.setHeader('Cache-Control', 'no-cache, no-store');
    } else if (path.includes('manifest.json') || path.includes('pwa.js')) {
...
// Serve root static files (index.html, styles.css, etc.)
app.use(express.static(__dirname, {
  setHeaders: (res, path) => {
    // Set proper headers for PWA files in root
...
// Specific route for service worker to ensure it's served from root
...
// Specific route for manifest to ensure it's served from root
